CREATE FUNCTION get_classes (schema_name text)
    RETURNS SETOF pg_catalog.pg_class
    AS $$
    SELECT
        *
    FROM
        pg_catalog.pg_class;

$$
LANGUAGE 'sql'
STABLE;

CREATE TABLE table_without_pg_catalog (
    bigint_type bigint,
    bigserial_type bigserial,
    bit_opt_1_type bit(1),
    bit_opt_2_type bit(2),
    bit_opt_16_type bit(16),
    bit_varying_opt_1_type varbit(1),
    bit_varying_opt_2_type varbit(2),
    bool_type bool,
    boolean_type boolean,
    box_type box,
    bytea_type bytea,
    char_opt_1_type char(1),
    char_opt_140_type char(140),
    char_opt_1000_type char(1000),
    char_type char,
    character_type character,
    character_varying_opt_1_type character varying(1),
    character_varying_opt_10_type character varying(10),
    character_varying_opt_140_type character varying(140),
    character_varying_opt_1000_type character varying(1000),
    character_varying_type character varying,
    cidr_type cidr,
    circle_type circle,
    date_type date,
    double_precision_type double precision,
    float_4_type float4,
    float_8_type float8,
    float_type float,
    float_opt_1_type float(1),
    float_opt_2_type float(2),
    float_opt_3_type float(3),
    float_opt_4_type float(4),
    float_opt_5_type float(5),
    float_opt_6_type float(6),
    float_opt_7_type float(7),
    float_opt_8_type float(8),
    geometry_point_type geometry(Point, 4326),
    geometry_polygon_type geometry(Polygon, 4326),
    inet_type inet,
    int_type int,
    int2_type int2,
    int4_type int4,
    int8_type int8,
    interval_year_type interval year,
    interval_month_type interval month,
    interval_day_type interval day,
    interval_hour_type interval hour,
    interval_minute_type interval minute,
    interval_type interval,
    json_type json,
    jsonb_type jsonb,
    line_type line,
    lseg_type lseg,
    macaddr_type macaddr,
    money_type money,
    numeric_type numeric,
    numeric_opt_type numeric(10,10),
    path_type path,
    pg_lsn_type pg_lsn,
    point_type point,
    polygon_type polygon,
    real_type real,
    serial_type serial,
    serial4_type serial4,
    serial8_type serial8,
    smallint_type smallint,
    text_type text,
    time_opt_2_type time(2) without time zone,
    time_type time,
    timestamp_type timestamp,
    timestamptz_type timestamptz,
    timetz_opt_2_type time(2) with time zone,
    timetz_type timetz,
    tsquery_type tsquery,
    tsvector_type tsvector,
    uuid_type uuid,
    varbit_1_type varbit(1),
    varbit_2_type varbit(2),
    varbit_16_type varbit(16),
    varbit_type varbit,
    varchar_opt_1_type varchar(1),
    varchar_opt_140_type varchar(140),
    varchar_opt_1000_type varchar(1000),
    varchar_type varchar,
    xml_type xml
);


CREATE TABLE table_with_pg_catalog (
    bigint_type pg_catalog.bigint,
    bigserial_type pg_catalog.bigserial,
    bit_opt_1_type pg_catalog.bit(1),
    bit_opt_2_type pg_catalog.bit(2),
    bit_opt_16_type pg_catalog.bit(16),
    bit_varying_opt_1_type pg_catalog.varbit(1),
    bit_varying_opt_2_type pg_catalog.varbit(2),
    bool_type pg_catalog.bool,
    boolean_type pg_catalog.boolean,
    box_type pg_catalog.box,
    bytea_type pg_catalog.bytea,
    char_opt_1_type pg_catalog.char(1),
    char_opt_140_type pg_catalog.char(140),
    char_opt_1000_type pg_catalog.char(1000),
    char_type pg_catalog.char,
    character_type pg_catalog.character,
    character_varying_opt_1_type character varying(1),
    character_varying_opt_10_type character varying(10),
    character_varying_opt_140_type character varying(140),
    character_varying_opt_1000_type character varying(1000),
    character_varying_type character varying,
    cidr_type pg_catalog.cidr,
    circle_type pg_catalog.circle,
    date_type pg_catalog.date,
    float_4_type pg_catalog.float4,
    float_8_type pg_catalog.float8,
    float_type pg_catalog.float,
    float_opt_1_type pg_catalog.float(1),
    float_opt_2_type pg_catalog.float(2),
    float_opt_3_type pg_catalog.float(3),
    float_opt_4_type pg_catalog.float(4),
    float_opt_5_type pg_catalog.float(5),
    float_opt_6_type pg_catalog.float(6),
    float_opt_7_type pg_catalog.float(7),
    float_opt_8_type pg_catalog.float(8),
    inet_type pg_catalog.inet,
    int_type pg_catalog.int,
    int2_type pg_catalog.int2,
    int4_type pg_catalog.int4,
    int8_type pg_catalog.int8,
    interval_year_type interval year,
    interval_month_type interval month,
    interval_day_type interval day,
    interval_hour_type interval hour,
    interval_minute_type interval minute,
    interval_type pg_catalog.interval,
    json_type pg_catalog.json,
    jsonb_type pg_catalog.jsonb,
    line_type pg_catalog.line,
    lseg_type pg_catalog.lseg,
    macaddr_type pg_catalog.macaddr,
    money_type pg_catalog.money,
    numeric_type pg_catalog.numeric,
    numeric_opt_type pg_catalog.numeric(10,10),
    path_type pg_catalog.path,
    pg_lsn_type pg_catalog.pg_lsn,
    point_type pg_catalog.point,
    polygon_type pg_catalog.polygon,
    --real_type pg_catalog.real,
    serial_type pg_catalog.serial,
    serial4_type pg_catalog.serial4,
    serial8_type pg_catalog.serial8,
    smallint_type pg_catalog.smallint,
    text_type pg_catalog.text,
    time_opt_2_type time(2) without time zone,
    time_type pg_catalog.time,
    time_type_mod pg_catalog.time(2),
    timestamp_type pg_catalog.timestamp,
    timestamptz_type pg_catalog.timestamptz,
    timetz_opt_2_type time(2) with time zone,
    timetz_type pg_catalog.timetz,
    tsquery_type pg_catalog.tsquery,
    tsvector_type pg_catalog.tsvector,
    uuid_type uuid,
    varbit_1_type pg_catalog.varbit(1),
    varbit_2_type pg_catalog.varbit(2),
    varbit_16_type pg_catalog.varbit(16),
    varbit_type pg_catalog.varbit,
    varchar_opt_1_type pg_catalog.varchar(1),
    varchar_opt_140_type pg_catalog.varchar(140),
    varchar_opt_1000_type pg_catalog.varchar(1000),
    varchar_type pg_catalog.varchar,
    xml_type pg_catalog.xml
);

CREATE TABLE table_with_pg_catalog_chars (
    a char,
    b bpchar
);

